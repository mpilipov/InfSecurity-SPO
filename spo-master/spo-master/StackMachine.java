        ...
        for (int i = 0; i < tokens.size(); i++) {
            ...
            switch (currentToken.getType()) {
                ...
                case "OPERATION_KW":
                    ...
                    switch(table.getVariableType(structName))
                    {
                        case "set": set = (MyHashSet) table.getVariableValue(structName);break;
                        case "list": list = (MyLinkedList) table.getVariableValue(structName);break;
                    }
                    if(list!=null)
                        switch (currentToken.getText()) {
                            ...
                            case "remove":
                                try {
                                    list.remove(Integer.valueOf(operand.toString()));
                        ...
                        switch (currentToken.getText()) {
                            ...
                            case "remove":
                                try {
                                    set.remove(Integer.valueOf(operand.toString()));
                                } catch (IndexOutOfBoundsException e) {
                                    System.out.println("Элемента с индексом " + operand + " не существует в списке "
                                            + structName);
                                    return;
                ...
                case "GET":
                    ...
                    list2 = (MyLinkedList) table.getVariableValue(stack.pop().getText());
                    try {
                        stack.push(new Token(String.valueOf(list2.get(index)), "CONST_FLOAT"));
                    } catch (IndexOutOfBoundsException e) {
